...
FROM gold.fact_sales; -- one order can have multiple products, so we need to count distinct order numbers

-- Find the total number of products
SELECT COUNT(product_name) AS total_products 
FROM gold.dim_products;
SELECT COUNT(DISTINCT product_key) AS total_products 
FROM gold.dim_products; -- same product can be sold multiple times
...
FROM gold.fact_sales; -- same customer can place multiple orders
